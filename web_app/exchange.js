...
const exchange_name = 'Cilantra';             // TODO: fill in the name of your exchange

const token_name = 'Cipher';                // TODO: replace with name of your token
const token_symbol = 'CIP';              // TODO: replace with symbol for your token
...
async function addLiquidity(amountEth, maxSlippagePct) {
    ...
    let rate = state.eth_token_rate;
    let min_rate = Math.ceil((rate - maxSlippagePct / 100) * mul);
    let max_rate = Math.floor((rate + maxSlippagePct / 100) * mul);
    let acc = provider.getSigner(defaultAccount);
    let amountToken = Math.floor(amountEth * state.token_eth_rate);

    await token_contract.connect(acc).approve(exchange_address, amountToken);
    //let allow = await token_contract.connect(acc).allowance(defaultAccount, exchange_address);
    await exchange_contract.connect(acc).addLiquidity(max_rate, min_rate, {value : ethers.utils.parseEther(amountEth.toString())});
...
async function removeLiquidity(amountEth, maxSlippagePct) {
    ...
    await exchange_contract.connect(acc).removeLiquidity(ethers.utils.parseEther(amountEth).toString(), max_rate, min_rate);
...
async function removeAllLiquidity(maxSlippagePct) {
    ...
    await exchange_contract.connect(acc).removeAllLiquidity(max_rate, min_rate);
...
async function swapTokensForETH(amountToken, maxSlippagePct) {
    ...
    await token_contract.connect(acc).approve(exchange_address, amountToken);
    await exchange_contract.connect(acc).swapTokensForETH(amountToken, max_rate); 
...
async function swapETHForTokens(amountEth, maxSlippagePct) {
   ...
   // console.log(max_rate);
    ...
    await exchange_contract.connect(acc).swapETHForTokens(max_rate, {value: ethers.utils.parseEther(amountEth.toString())});
...
const sanityCheck = async function() {
 ...
 // console.log(start_state);
  ...
  // LP provider rewards implemented
     ...
     // console.log(start_state.token_eth_rate);
      var user_tokens1 = await token_contract.connect(provider.getSigner(defaultAccount)).balanceOf(defaultAccount);
      score += check("Testing simple exchange of ETH to token", swap_fee[0], 
      Math.abs((start_state.token_liquidity - expected_tokens_received) - state1.token_liquidity) < 5 &&
      (state1.eth_liquidity - start_state.eth_liquidity) === 100 &&
      Math.abs(Number(start_tokens) + expected_tokens_received - Number(user_tokens1)) < 5);
      // console.log(expected_tokens_received);
      // console.log(start_state.token_liquidity);
      // console.log(state1.token_liquidity);
      //console.log(Number(user_tokens1));
      ...
      score += check("Test simple exchange of token to ETH", swap_fee[0], 
        ...
        Math.abs((state1.eth_liquidity - expected_eth_received) - state2.eth_liquidity) < 5 &&
        Number(user_tokens2) === (Number(user_tokens1) - 90));
      //console.log(expected_eth_received);
      defaultAccount = accounts[0];
      var state2 = await getPoolState();
      user_tokens2 = await token_contract.connect(provider.getSigner(defaultAccount)).balanceOf(defaultAccount);
